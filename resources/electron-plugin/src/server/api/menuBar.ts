    ...
    if (onlyShowContextMenu) {
        ...
        state.activeMenuBar.on("ready", () => {
            ...
            state.activeMenuBar.on("hide", () => {
                notifyPhp("events", {
                    event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarHidden"
                });
            });

            state.activeMenuBar.on("show", () => {
                notifyPhp("events", {
                    event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarShown"
                });
            });
...
function eventsForTray(tray, onlyShowContextMenu, contextMenu, shouldSendCreatedEvent) {
    ...
    if (shouldSendCreatedEvent) {
        notifyPhp("events", {
            event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarCreated"
        });
    }

    tray.on("drop-files", (event, files) => {
        notifyPhp("events", {
            event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarDroppedFiles",
            payload: [
                files
            ]
        });
    ...
    tray.on('click', (combo, bounds, position) => {
        notifyPhp('events', {
            event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarClicked",
            payload: {
                combo,
                bounds,
                position,
    ...
    tray.on("right-click", (combo, bounds) => {
        notifyPhp("events", {
            event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarRightClicked",
            payload: {
                combo,
                bounds,
            }
    ...
    tray.on('double-click', (combo, bounds) => {
        notifyPhp('events', {
            event: "\\Native\\Nette\\Events\\MenuBar\\MenuBarDoubleClicked",
            payload: {
                combo,
                bounds,
            },
