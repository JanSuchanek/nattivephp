    ...
    try {
        ...
        const startTimeout = setTimeout(() => {
            if (!state.processes[alias] || !state.processes[alias].pid) {
                ...
                notifyPhp('events', {
                    event: 'Native\\Nette\\Events\\ChildProcess\\StartupError',
                    payload: {
                        alias,
        ...
        proc.stdout.on('data', (data) => {
            notifyPhp('events', {
                event: 'Native\\Nette\\Events\\ChildProcess\\MessageReceived',
                payload: {
                    alias,
                    data: data.toString(),
                }
        ...
        proc.stderr.on('data', (data) => {
            ...
            notifyPhp('events', {
                event: 'Native\\Nette\\Events\\ChildProcess\\ErrorReceived',
                payload: {
                    alias,
                    data: data.toString(),
                }
        ...
        proc.on('spawn', () => {
            ...
            notifyPhp('events', {
                event: 'Native\\Nette\\Events\\ChildProcess\\ProcessSpawned',
        ...
        proc.on('exit', (code) => {
            ...
            notifyPhp('events', {
                event: 'Native\\Nette\\Events\\ChildProcess\\ProcessExited',
                payload: {
                    alias,
                    code,
                }
...
function startPhpProcess(settings) {
    ...
    if (settings.cmd[0] === 'bin/console' && runningSecureBuild()) {
