...
mkdirpSync(join(storagePath, 'temp'));
...
async function retrievePhpIniSettings() {
    ...
    let command = ['bin/console', 'native:php-ini'];
...
async function retrieveNativePHPConfig() {
    ...
    let command = ['bin/console', 'native:config'];
...
function callPhp(args, options, phpIniSettings = {}) {
    ...
    if (args[0] === 'bin/console' && runningSecureBuild()) {
...
function startScheduler(secret, apiPort, phpIniSettings = {}) {
    ...
    return callPhp(['bin/console', 'schedule:run'], phpOptions, phpIniSettings);
}
...
function getDefaultEnvironmentVariables(secret?: string, apiPort?: number): EnvironmentVariables {
    ...
    // Nette doesn't need Laravel-specific cache paths
...
function serveApp(secret, apiPort, phpIniSettings): Promise<ProcessResult> {
    return new Promise(async (resolve, reject) => {
        ...
        console.log('Starting PHP server...', `${state.php} -S`, appPath, phpIniSettings)
        ...
        // Nette doesn't need artisan optimize

        // Nette database migrations can be handled separately if needed
        ...
        if (runningSecureBuild()) {
            ...
            console.log('* * * Running Nette from source * * *');
            serverPath = join(appPath, 'www', 'index.php');
            cwd = join(appPath, 'www');
        }
